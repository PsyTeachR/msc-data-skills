...
```{r ex-1b}
study1 <- left_join(disgust, users, by = "user_id") %>%
  glimpse()
```
...
```{r ex-2}
study2 <- right_join(users, disgust, by = "user_id") %>%
  glimpse()
```
...
```{r ex-3}
study3 <- inner_join(disgust, personality, 
                     by = c("user_id", "date")) %>%
  glimpse()
```
...
```{r ex-3-i}
study3_nodate <- inner_join(disgust, personality, 
                            by = c("user_id")) %>%
  glimpse()
```
...
```{r ex-4}
study4 <- full_join(disgust, personality, 
                    by = c("user_id", "date")) %>%
  glimpse()
```
...
```{r ex-5}
study5 <- semi_join(disgust, personality, 
                    by = c("user_id", "date")) %>%
  glimpse()
```
...
```{r ex-6}
study6 <- users %>%
  anti_join(personality, by = "user_id") %>%
