...
Make a violin plot with `breed` on the x-axis and `weight` on the y-axis. Make each violin white, 50% transparent, and outlined in a different coloour, but omit the legend.
...
Make a boxplot with `breed` on the x-axis and `weight` on the y-axis. Make each violin white, 50% transparent, and outlined in a different coloour, but omit the legend.
...
Make a density with `weight` on the x-axis. Make each breed white, 50% transparent, and outlined in a different colour, but omit the legend.
...
Use `stat_summary` to create a column plot with error bars showing 1 standard error. Make each column white with a black border, but omit the legend.
...
dog_violin + colours + labs +
  ggtitle("Violin Plot")
ggsave("dog_violin.png")

dog_boxplot + colours + labs +
  ggtitle("Box Plot")
...
Use `stat_density_2d(aes(fill = ..level..), geom = "polygon", n = n)`, set n to a value that makes the graph look good, and 
