...
<div class="warning">
<p>If you loaded the {MASS} package and run the code above without putting <code>dplyr::</code> before the <code>select()</code> function, you might get an error message. This is because the <code>MASS</code> package also has a function called <code>select()</code> and, if you loaded <code>MASS</code> after <code>tidyverse</code>, the <code>MASS</code> version of the function becomes the default. After loading <code>MASS</code>, you should see a warning like “The following object is masked from ‘package:dplyr’: select”. You can use functions with the same name from different packages by specifying the package before the function name, separated by two colons.</p>
</div>
