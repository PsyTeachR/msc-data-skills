...
`ggplot2` creates plots using a "grammar of graphics" where you add `r glossary("geoms")` in layers. It can be complex to understand, but it's very powerful once you have a mental model of how it works. 

Let's start with a totally empty plot layer created by the `ggplot()` function with no arguments.

```{r ggplot_empty, fig.cap="A plot base created by ggplot()"}
ggplot()
```
...
```{r ggplot_labels, fig.cap="Empty ggplot with x and y labels"}
mapping <- aes(x = pet, 
               y = score, 
               colour = country, 
               fill = country)
...
```{r ggplot_geom, fig.cap="Violin plot with country represented by colour."}
...
```{r colplot_statsum, fig.cap="Column plot"}
...
```{r vertint, fig.cap="Different vertical interval geoms"}
...
```{r vertint_statssum, fig.cap="Vertical intervals with stats_summary()"}
...
```{r custom_themes, fig.cap="Custom theme"}
