...
Q[2] <- c(all.equal(regal, c("R", "E", "G", "A", "L")),
          ...
          all.equal(lager, "LAGER"))
Q[3] <- c(all.equal(nrow(ss), 27021L),
          all.equal(ncol(ss), 17L))
Q[4] <- c(all.equal(nrow(ss_long), 378294L),
          all.equal(ncol(ss_long), 5L))
Q[5] <- c(all.equal(nrow(ss_sep), 378294L),
          all.equal(ncol(ss_sep), 6L),
          all.equal(names(ss_sep), c("id", "user_id", "date", "domain", "qnumber", "score")))
Q[6] <- c(all.equal(nrow(ss_unite), 378294L),
          all.equal(ncol(ss_unite), 5L),
          all.equal(names(ss_unite), c("super_id", "date", "domain", "qnumber", "score")))
Q[7] <- c(all.equal(nrow(ss_wide), 27021L),
          all.equal(ncol(ss_wide), 17L))
Q[8] <- c(c("sibage", "sibsex") %in% names(family), nrow(family) == 115014)
Q[9] <- c(c("face_id", "description") %in% names(eyes),
          if(is.data.frame(eyes)) count(eyes, description) %>% filter(n == max(n)) %>% pull(description) == "brown" else FALSE)
